...
WARNING: All configuration options described in this file should go into the thruk_local.conf
...
A menu has the following attributes:
     ...
     ** `icon`          icon for the menu itself. You can use `{{theme}}` as placeholder in the url
                        and `{{remote_user}}` for the user name.
...
The menu item can have the following attributes:
     ...
     ** `icon`          icon for the menu item. You can use `{{theme}}` as placeholder in the url.
...
Sample icons with two action icons:
  ...
  [
    {
      "icon":"/thruk/themes/{{theme}}/images/criticity_1.png",
...
Variant 1 - reference to a pre defined menu:
...
Variant 2 - define the menu in object config only:
...
ex.:
  ...
  <action_menu_apply>
    standardmenu = .*;                # matches all services on all hosts
    specialmenu  = ^Host;Service$     # matches an exact service on one host only
    cpu_menu     = ^.*;CPU            # matches all services starting with 'CPU' on all hosts
  ...
  <Component Thruk::Backend>
    <peer>
        ...
        <configtool>
            ...
            obj_check  = /etc/init.d/naemon checkconfig
            obj_reload = /etc/init.d/naemon reload
        </configtool>
    </peer>
    <peer>
...
probably have to download the link:/documentation/faq.html#wkhtmltopdf[wkhtmltopdf binary] in a recent version.
...
See the link:/documentation/faq.html#wkhtmltopdf[FAQ] for download
