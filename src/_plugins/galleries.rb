# Jekyll GalleryTag (https://github.com/internaut/JekyllGalleryTag)
#
...
module Jekyll
    ...
    class GalleryTag < Liquid::Block
        ...
        def render(context)
            ...
            images = gallery_images(context.registers[:site], contents)
        ...
        def gallery_images(site, contents)
            ...
            input_data.each do |item|
                full_item_path = File.join(site.source, source_dir, item[0])
    ...
    class ThumbGenerator < Generator
        ...
        def generate(site)
            ...
            @gallery_dir  = File.expand_path(site.source + "/" + (@config['source_dir'] != nil ? @config['source_dir'] : (@config['dir'] != nil ? @config['dir'].sub(/^\//, '') : "images/gallery")))
